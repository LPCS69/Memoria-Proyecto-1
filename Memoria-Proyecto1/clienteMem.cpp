...
void clienteMem::Enviar()
{
    ...
    //*buffer = NULL;
   // if (tarjetas_destapadas.length()>2){tarjetas_destapadas.clear();}
   // if (imagenes_destapadas.length()>2){imagenes_destapadas.clear();}
...
void clienteMem::posicion_aux(int num) {
    if (conectado) {
        if (!enviando) {
                ...
                //if (!tarjetas_destapadas.contains(num) & tarjetas_destapadas.length()<limite_destape ){
...
bool clienteMem::encontrar_pareja() {
...
void clienteMem::quitar_pareja(int num) {
...
void clienteMem::girar_tarjeta(int numero_carta, QString imagen_carta) {
